...
pub struct TextMeasurer {
    font: Font,
    stb_font: Option<FontInfo<Vec<u8>>>,
}

impl TextMeasurer {
    pub fn new(font_data: &[u8]) -> Option<Self> {
        for collection_index in 0..32 {
            let settings = fontdue::FontSettings {
                collection_index,
                scale: 40.0,
                load_substitutions: true,
            };
            let Ok(font) = Font::from_bytes(font_data, settings) else {
                continue;
            };

            let stb_font = stb_truetype::get_font_offset_for_index(font_data, collection_index as i32)
                .and_then(|offset| FontInfo::new(font_data.to_vec(), offset as usize));

            return Some(Self { font, stb_font });
        }
    ...
    pub fn from_font_with_data(font: Font, font_data: &[u8]) -> Option<Self> {
        let stb_font = stb_truetype::get_font_offset_for_index(font_data, 0)
            .and_then(|offset| FontInfo::new(font_data.to_vec(), offset as usize));
    ...
    pub fn measure_char_advance(&self, ch: char, font_size: f32) -> f32 {
        if self.stb_font.is_none() {
            let px = self.fontdue_render_font_size(font_size);
            return self.font.metrics(ch, px).advance_width.max(0.0);
        }

        let stb_font = self.stb_font.as_ref().unwrap();
        let px = (font_size * 1.20_f32).round();
        let scale = stb_font.scale_for_pixel_height(px);
        let cp = ch as u32;
        let glyph = stb_font.find_glyph_index(cp);
        let glyph = if glyph == 0 && cp != 0 {
            stb_font.find_glyph_index('?' as u32)
        } else {
            glyph
        };
        if glyph == 0 {
        ...
        let hm = stb_font.get_glyph_h_metrics(glyph);
    ...
    pub fn measure_width(&self, text: &str, font_size: f32) -> f32 {
        if self.stb_font.is_none() {
            ...
            return text
                .chars()
                .map(|ch| self.font.metrics(ch, px).advance_width.max(0.0))
                .sum();
        }
    ...
    pub fn stb_to_fontdue_ratio(&self) -> f32 {
        let Some(stb_font) = self.stb_font.as_ref() else {
            return 0.0;
        };

        let vm = stb_font.get_v_metrics();
    ...
    fn fontdue_render_font_size(&self, cmd_font_size: f32) -> f32 {
        let ratio = self.stb_to_fontdue_ratio();
        if ratio > 0.0 {
            (cmd_font_size * 1.20_f32).round() * ratio
        } else {
            cmd_font_size
        }
    }
